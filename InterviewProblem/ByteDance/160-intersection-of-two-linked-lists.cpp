...
public:
    ...
    ListNode *getIntersectionNode(ListNode *headA, ListNode *headB) {
        ...
        while (currA != currB){
            ...
            if (currA == nullptr && currB == nullptr && second_run) {
                return nullptr;
            }
            if (currA == nullptr) {
                currA = headB;
 ...
 * Note:
 *
 * 双指针经典场景：两条链表 O(n)时间内找到相交的链接处
 * 指针A、B分别从链表A、B头节点出发，走完自身链表后从对方头节点出发，
 * A、B指针相遇处即为两条链表链接处，如果不相遇，A、B都跑到了nullptr，则说明两条链表不相交
 */